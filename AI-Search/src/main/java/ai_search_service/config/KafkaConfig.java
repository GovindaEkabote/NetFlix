...
public class KafkaConfig {
    ...
    public ConsumerFactory<String, VideoEncodedEvent> consumerFactory() {
        ...
        // Producer is NOT sending type headers
        props.put(JsonDeserializer.USE_TYPE_INFO_HEADERS, false);

        // Tell Spring which class to deserialize into
        props.put(
                JsonDeserializer.VALUE_DEFAULT_TYPE,
                VideoEncodedEvent.class.getName()
        );

        return new DefaultKafkaConsumerFactory<>(
                props,
                new StringDeserializer(),
                new JsonDeserializer<>(VideoEncodedEvent.class, false)
        );
    }
